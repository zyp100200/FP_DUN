...
model.to(device)
# input = torch.randn(64, 3, 32, 32).to(device)
# model(input)
print("\n")
# tw.draw_model(model, [1,1,224,224])

# summary(model,(1,1,224,512))
        ...
        with torch.no_grad():
            ...
            for i in range(len(filelist)):
                if os.path.splitext(filelist[i])[1] in ['.tif', '.bmp', '.png', '.jpg']:
                    ...
                    # print(a,b,c,d)
                    # summary(model,input_size=(a,b,c,d))
            ...
            print("平均时间：",sum(times)/len(times))
